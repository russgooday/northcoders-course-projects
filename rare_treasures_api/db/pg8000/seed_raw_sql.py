''' Seeds the database using the standard PG8000.native api. '''
import json
...
def seed_db(env='test'):
    ...
    with CreateConnection() as db:
        ...
        if not (shops := db.run('SELECT * FROM shops')):
            print('Nothing to seed! Please check the `shops` table.')
            return
        ...
        with open(treasures_path, 'r', encoding='utf-8') as file:
            ...
            print(f'Seeded {row_count} rows to `treasures` table in the database. \U0001F44D')
